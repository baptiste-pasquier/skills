...
the script:
  ...
  * GENERATED_BACKLOG_HEADER = None   - no generated backlog mirror (drop "BACKLOG.md"
                                        from ROOT_ALLOWED too)
...
#: Files at the root of `docs/` that are not quadrant docs. Drop "BACKLOG.md" when the
#: project has no backlog mirror - and set GENERATED_BACKLOG_HEADER to None with it, since
#: the two describe the same file.
...
#: Set to None if the project has no generated backlog mirror, and drop "BACKLOG.md" from
#: ROOT_ALLOWED at the same time. A mirror is worth having only when a workflow refreshes
#: it automatically - see the skill's references/backlog.md.

...
the script:
  ...
  * GENERATED_BACKLOG_HEADER = None   - no generated backlog mirror; the root
                                        allowlist follows automatically
...
#: the mirror. Empty in most projects: a doc without a folder has no home.
...
#: The header the mirror's generator writes. **Set this to None when the project
#: has no backlog mirror** - it is the single switch, and `root_allowed()` stops
#: tolerating the file at the same moment. A mirror is worth having only when a
#: workflow refreshes it automatically; see the skill's references/backlog.md.
GENERATED_BACKLOG_HEADER: str | None = "<!-- GENERATED by scripts/sync_backlog.py"
